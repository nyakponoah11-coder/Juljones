...
|--------------------------------------------------------------------------
|
| Every branch has its own WhatsApp number.
|
| Put the real WhatsApp number for each branch in Render Environment
| Variables.
|
*/
...
|--------------------------------------------------------------------------
|
| Customer chooses any base amount from 10 to 100.
|
*/
...
|
| IMPORTANT:
| Render can restart your application, which clears these.
| For production we can later move this to Supabase.
|
*/
...
|--------------------------------------------------------------------------
|
| Because WhatsApp lists don't support checkboxes, the customer selects
| one protein at a time.
|
| After selecting it, we ask whether they want another.
|
*/
...
function getBaseFoodPrice(session) {
  if (
    session.food === "Fried Rice" ||
    session.food === "Jollof Rice"
  ) {
